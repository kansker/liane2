		...
		for (var j = 0; j < this.imagerList.length; j++) {
			if (this.imagerList[j] != null && this.imagerList[j].pos == pos) {
				...
				xml += ('<imager viewID="' + this.imagerList[j].viewID
						...
						+ '" newURI="' + this.imagerList[j].newURI + '" left="'
						+ dobj.x + '" top="' + dobj.y + '" width="' + dobj.w
						+ '" height="' + dobj.h + '" swidth="'
						+ this.imagerList[j].swidth + '" sheight="'
						+ this.imagerList[j].sheight + '"></imager>');
			}
		}
		return xml;
	},
...
KPHOTO.prototype.delImager = function(id) {
	for (var j = 0; j < this.imagerList.length; j++) {
		if (this.imagerList[j] != null && this.imagerList[j].id == id) {
			...
			this.imagerList = this.imagerList.slice(0, j)
					.concat(this.imagerList.slice(j + 1));
			this.sid = "";
			break;
		}
	}
	...
	for (var i = 0; i < this.smap.sids.length; i++) {
		...
		if (this.smap.sids[i].indexOf("imager") == 0) {
			...
			if (obj != null) {
				...
				kphoto.chooseAlbum($j('#nowCategory').val());

...
function my_DragFunc() {
    if( pickedItem== null){
        if (dd.obj.name.indexOf("plane") == 0) {
            return;
        }
        if (dd.obj == rb)
        {
...
function dropCellByCalendar() {
	var cell = null;
	var area = -1;
	try{
        var overlap = boxOverlap(dd.obj, dd.elements["plane1Image"]);
        if (overlap > area) {
            area = overlap;
            cell = dd.elements["plane1Image"];
        }
	}catch(e){
        alert(e.description);
	}
...
function my_DropFunc() {
    ...
    if(pickedItem.viewID == "text-rect"){
		...
		if (toCell != null) {
            if (toCell.name.indexOf("plane") == 0) {
                var obj1 = albumObj(pickedItem.viewID);
                $('plane1_img').src = obj1.viewURI;
                plane1.viewURI = obj1.viewURI;
                plane1.viewID = obj1.viewID;
            }
            else
            {
...
function setDraggableImages(dragOn) {
	...
	for (var i=0; i < imageDropZones.length; i++){
        ...
        //alert(imageDropZones[i].name+" "+dd.elements[imageDropZones[i].name].nodrag);
    }
	setTimeout("albumPage("+!dragOn+")", 10);
	dd.dragOn = dragOn;
}
function setDraggableImages2(dragOn) {

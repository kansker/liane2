...
public class Stylecgy extends EventAction {
	...
	public String add() {
		...
		this.setSession("Stylecategory", data);
		return SUCCESS;
	}

	public String update() {
		this.createToken();
		data = QueryAgent.query("q_stylecategory", condition);
		if (data != null) {
	...
	public String save() {
		...
		this.useToken();
		WParam nowData = this.getSessionWParam("Stylecategory");
		nowData.set(data);
		...
		if (nowData.equals("method", "add")) {
			agent.executeUpdate("add_stylecategory", nowData);
		} else {
			agent.executeUpdate("update_stylecategory", nowData);
		}
	...
	public String remove() {
		...
		DBAgent agent = new DBAgent();
		agent.startTransaction();
		agent.executeUpdate("del_stylecategory", condition);
		boolean r = agent.endTransaction();
		if (r) {
			this.addCmd("refresh();");
			this.alert("成功");
	...
	public String datas() {
		...
		List<WParam> list = QueryAgent.queryList("qa_stylecategory", condition, condition.getInt("offset"), condition.getInt("limit"), order);
		try {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				try {
					...
					json.put("forWho", CodeLoader.loadCodeValue("forWho", "v", "c", dd.getString("forWho")));
				} catch (Exception e) {
					e.printStackTrace();
				}
				jlist.put(json);

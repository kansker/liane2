...
public class Pcgy1 extends EventAction {
	private static final long serialVersionUID = 2024628204419943134L;
	private String prefix = "admin_pcgy1";
	private WParam data;
	...
	public String update() {
		this.createToken();
		data = QueryAgent.query("q_productcgy1", condition);
		if (data != null) {
	...
	public String save() {
		...
		if (nowData.equals("method", "add")) {
			agent.executeUpdate("add_productcgy1", nowData);
		} else {
			agent.executeUpdate("update_productcgy1", nowData);
		}
	...
	public String remove() {
		...
		DBAgent agent = new DBAgent();
		agent.startTransaction();
		agent.executeUpdate("del_productcgy1", condition);
		boolean r = agent.endTransaction();
		if (r) {
			this.addCmd("refresh();");
			this.alert("成功");
	...
	public String datas() {
		...
		List<WParam> list = QueryAgent.queryList("qa_productcgy1", condition, condition.getInt("offset"), condition.getInt("limit"), order);
		try {

...
public class Framescgy extends EventAction {
	...
	public String add() {
		...
		this.setSession("Framescgy", data);
		return SUCCESS;
	}

	public String update() {
		this.createToken();
		data = QueryAgent.query("q_frames_cgy", condition);
		if (data != null) {
	...
	public String save() {
		...
		this.useToken();
		WParam nowData = this.getSessionWParam("Framescgy");
		nowData.set(data);
		...
		if (nowData.equals("method", "add")) {
			agent.executeUpdate("add_frames_cgy", nowData);
		} else {
			agent.executeUpdate("update_frames_cgy", nowData);
		}
	...
	public String remove() {
		...
		DBAgent agent = new DBAgent();
		agent.startTransaction();
		agent.executeUpdate("del_frames_cgy", condition);
		boolean r = agent.endTransaction();
		if (r) {
			this.addCmd("refresh();");
			this.alert("成功");
	...
	public String datas() {
		...
		List<WParam> list = QueryAgent.queryList("qa_frames_cgy", condition, condition.getInt("offset"), condition.getInt("limit"), order);
		try {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				try {
					...
					json.put("cgyName", dd.getString("cgyName"));
